...
class CampusModel {
  ...
  async createCampus(campusData) {
    ...
    const sql = `
      INSERT INTO campuses (campus_name, location, status)
      VALUES (?, ?, ?)
    `;
  ...
  async getAllCampuses() {
    const sql = `
      SELECT 
        ...
        created_at,
        updated_at,
        (SELECT COUNT(*) FROM clinics WHERE campus_id = campuses.campus_id) as clinic_count
      FROM campuses
  ...
  async updateCampus(campusId, campusData) {
    ...
    const sql = `
      UPDATE campuses 
      SET campus_name = ?, location = ?, status = ?, updated_at = CURRENT_TIMESTAMP
      WHERE campus_id = ?
    `;
  ...
  // Delete campus (soft delete by setting status to Inactive)
